...
const getSettlementsCalculatedService = async (groupCode) => {
  const group = await Group.findOne({ [COMMON_FIELDS.GROUP_CODE]: groupCode })
    .select(GROUP_FIELDS.SETTLEMENTS_CALCULATED)
    .lean();
  ...
  return Boolean(group[GROUP_FIELDS.SETTLEMENTS_CALCULATED]);
};

export default getSettlementsCalculatedService;
